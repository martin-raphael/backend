    ...
    .then((result) => {
      if (result.length > 0) {
        ...
        if (expiresAt < Date.now()) {
          ...
          UserVerification.deleteOne({ userId })
            .then((result) => {
              User.deleteOne({ _id: userId })
                ...
                .catch((error) => {
                  console.log(error);
                  let message = "Clearing User with expired unique link";
                  res.redirect(`/user/verified/error=true$message=${message}`);
                });
            })
            .catch((error) => {
          ...
          // valid record still exists so we validate the user.
          //comparing hashed uniques strings before continue
          bcrypt
            ...
            .catch((error) => {
              console.log(error);
              let message = "An error occurred while comparing uniques strings";
              res.redirect(`/user/verified/error=true$message=${message}`);
            });
        }
      } else {
